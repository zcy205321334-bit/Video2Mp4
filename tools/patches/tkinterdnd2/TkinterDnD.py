...
try:
    ...
    try:
        ...
        tix = None  # tix 在 Python 3.13+ 已移除；本应用仅用 TkinterDnD.Tk，无需 tix
...
if tix is not None:
    ...
    class TixTk(DnDWrapper):
        '''tix 在 Python 3.13+ 已移除时的占位类；本应用仅用 TkinterDnD.Tk，不使用 TixTk。'''
        def __init__(self, *args, **kw):
            raise RuntimeError("TixTk 需要 tix 扩展（Python 3.13+ 已移除）")
